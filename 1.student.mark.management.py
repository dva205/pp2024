...
def input_mark():
    ...
    for student in students:
        mark = float(input(f"Enter mark for student {student['Name']} ID: {student['ID']}: ")) # {student["Name"]} +  ID:  + {student["ID"]} get Name and ID of the student that in dict student
        marks[id_course][student["ID"]] = mark # key value is id_course and value is a dictionary with key-value is student["ID"] 
...
def show_mark():
    ...
    for student in students:
        ...
        if student["ID"] in marks[id_course]:
            print(f"Mark for the student {student['ID']} Name: {student['Name']} in this {id_course}: {marks[id_course][id_student]}")
def menu():
    while True:
        print("\n------ Menu ------")
